...
MPU9250 accelgyro;
I2Cdev   I2C_M;
SoftwareSerial MyBlue(2, 4); // RX | TX + CONNECT KEY PIN TO V5
uint8_t buffer_m[6];
...
const
int redPin = 6;  // R petal on RGB LED

const
int greenPin = 5;  // G petal on RGB LED

const
int bluePin = 3;  // B petal on RGB LED
...
int get_dir() {
  float arr[6][3] = {{10, 4.1, 2.5}, { -6.5, 3, -5.5}, { -7.5, 15, -5}, { -8, 18.5, -19.5}, { -7, 6.3, -22}, {11, 3.5, -27}};
  int min_loc = -1;
  float min_dist = 1000;
  for (int i = 0; i < 6 ; i++) {
    float cur_dist = abs(Mxyz[0] - arr[i][0]) + abs(Mxyz[1] - arr[i][1]) + abs(Mxyz[2] - arr[i][2]);
